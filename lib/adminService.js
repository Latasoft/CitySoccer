...
export const pricesService = {
  ...
  // Actualizar múltiples precios (OPTIMIZADO)
  updateBatch: async (precios) => {
    try {
      ...
      // Si solo hay 1-5 cambios, usar updates individuales en paralelo (MÁS RÁPIDO)
      if (precios.length <= 5) {
        console.log('🔍💰 Modo FAST: Updates individuales en paralelo');
        
        const timestamp = new Date().toISOString();
        const updatePromises = precios.map(precio => 
          supabase
            .from('precios')
            .update({ 
            ...
            .eq('id', precio.id)
            .select('id') // Solo retornar ID, no todo el registro
        );
        ...
        if (errors.length > 0) {
          console.error('🔍💰 Errores en updates individuales:', errors);
          throw errors[0].error;
        }
        
        console.log('🔍💰 ✅ Updates individuales completados:', precios.length);
        return { data: precios, error: null };
      }
      
      // Si hay 6+ cambios, usar upsert batch
      console.log('🔍💰 Modo BATCH: Upsert masivo');
      
      const updates = precios.map(precio => ({
        id: precio.id,
        tipo_cancha: precio.tipo_cancha,
        dia_semana: precio.dia_semana,
        hora: precio.hora,
        precio: precio.precio,
        activo: precio.activo,
        actualizado_en: new Date().toISOString()
        // NO incluir creado_en (innecesario en update)
      }));
      
      console.log('🔍💰 Ejecutando upsert de', updates.length, 'registros...');

      const { data, error } = await supabase
        .from('precios')
        .upsert(updates, { 
          onConflict: 'id',
          ignoreDuplicates: false // Forzar actualización
        })
        .select('id') // Solo retornar IDs, no registros completos
        .select();
