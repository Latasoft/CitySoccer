  ...
  try {
    ...
    const { data, error } = await supabase.storage
      ...
      .upload(fileName, blob, {
        ...
        cacheControl: '0', // SIN CACHE - contenido editable debe ser siempre fresh
      });
...
export async function getContent(pageKey) {
  try {
    ...
    // CRÍTICO: Usar getPublicUrl + fetch con cache-busting para bypass del CDN
    const { data: urlData } = supabase.storage
      .from(CONTENT_BUCKET)
      .getPublicUrl(fileName);

    if (!urlData || !urlData.publicUrl) {
      console.error(`❌ [contentStorage] No se pudo obtener URL pública para ${pageKey}`);
      return null;
    }

    // Agregar timestamp para cache-busting (bypass del CDN de Supabase)
    const urlWithCacheBust = `${urlData.publicUrl}?t=${Date.now()}`;
    
    console.log(`🔍 [contentStorage] Fetching con cache-bust: ${urlWithCacheBust.substring(0, 100)}...`);

    // Fetch con headers que fuerzan bypass de cache
    ...
    if (!response.ok) {
      ...
      console.error(`❌ [contentStorage] Error HTTP ${response.status} descargando ${pageKey}`);
      return null;
    }
  ...
  try {
    ...
    const { data, error } = await supabase.storage
      ...
      .upload(fileName, file, {
        contentType,
        cacheControl: '3600', // Cache por 1 hora (antes 1 año)
    ...
    // Agregar timestamp para cache-busting (fuerza recarga en navegador)
    const urlWithCacheBust = `${urlData.publicUrl}?v=${Date.now()}`;

    console.log(`✅ [uploadFile] Archivo subido: ${fileName}`);
    console.log(`✅ [uploadFile] URL pública: ${urlWithCacheBust}`);

    return {
      success: true,
      url: urlWithCacheBust, // Retornar URL con cache-busting
      path: data.path,
      fileName: fileName
    };
  } catch (error) {
