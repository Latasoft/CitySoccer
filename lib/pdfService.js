...
export const generateReservationPDF = async ({
  ...
  try {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(12);
    doc.setTextColor(146, 64, 14); // Café oscuro
    doc.text('📌 INSTRUCCIONES IMPORTANTES', margin + 5, currentY + 8);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    const instrucciones = [
      '• Llega 10 minutos antes de tu horario reservado',
      '• Presenta este comprobante o el código QR en recepción',
      '• En caso de no asistir, comunícate con anticipación'
    ];

    let instructionY = currentY + 15;
    instrucciones.forEach(inst => {
      doc.text(inst, margin + 5, instructionY);
      instructionY += 5;
    });
    ...
    doc.text('📍 Dirección de las canchas • 📞 +56 9 1234 5678', pageWidth / 2, footerY + 13, { align: 'center' });
